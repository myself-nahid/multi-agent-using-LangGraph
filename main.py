...
def validate_agent_type(agent_type: str) -> str:
    ...
    if cleaned_type in CLEANED_AGENT_NAMES_MAP:
        ...
        raise HTTPException(status_code=400, detail=f"Invalid agent_type '{agent_type}'. Valid options are: {AGENT_NAMES}")
...
# --- THE ROBUST CORS FIX ---
# The wildcard "*" tells the browser that requests from ANY origin are allowed.
# This is safe for local development but should be restricted to your
# specific frontend domain name when you deploy to production.
app.add_middleware(
    CORSMiddleware,
